...
async function guardarProducto(e) {
  ...
  if (!data.codigo || data.codigo.trim() === '') {
    console.error('❌ Error: Código del producto vacío');
    alert('Error: El código del producto no puede estar vacío');
    return;
  }
...
async function editarProducto(codigo) {
  try {
    ...
    document.getElementById('codigo').readOnly = true; // No permitir cambiar el código al editar
...
async function eliminarProducto(codigo) {
  ...
  if (!codigo || codigo === 'null' || codigo === 'undefined' || codigo.trim() === '') {
    alert('Error: El producto no tiene un código válido para eliminar.');
    return;
  }
...
function limpiarFormulario() {
  ...
  document.getElementById('codigo').readOnly = false; // Habilitar edición del código para nuevo producto
